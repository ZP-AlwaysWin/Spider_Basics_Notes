...
其中它包含一个数字"状态码"。举个例子，假如response是一个"重定向"，需定位到别的地址获取文档，urllib将对此进行处理。
...
下面我们写一个例子来感受一下，捕获的异常是HTTPError，它会带有一个code属性，就是错误代号，
另外我们又打印了reason属性，这是它的父类URLError的属性。


import urllib.request
import urllib.error

req = urllib.request.Request('http://blog.csdn.net/Always_Win')
try:
...
如果捕获到了HTTPError，则输出code，不会再处理URLError异常。如果发生的不是HTTPError，则会去捕获URLError异常，
输出错误原因。
